...
const QRScanner = ({ isBatch, html5QrCode }) => {
    ...
    const qrCodeSuccessCallback = (decodedText, decodedResult) => {
        isBatch ? dispatch(scann.batchScan(decodedResult)) : dispatch(scann.coiScan(decodedResult))
        html5QrCode.pause()
        stopScanning();
    }
    ...
    return (
        <>
            <div className="flex items-center w-full justify-center h-1/5">
                {!scannerState ? <button className="rounded-lg bg-purple-700 py-2 px-2 text-white hover:bg-purple-500" onClick={() => startScanning()}>Start Scan</button> :
                    <button className="rounded-lg bg-purple-700 py-2 px-2 text-white hover:bg-purple-500" onClick={() => stopScanning()}>Stop Scan</button>}
            </div>
        </>
    )
}
